...
def prediction(image_path):
    ...
    disease_label = model.config.id2label[pred]

    # Write the output to a file
    output_file_path = os.path.join("static", "output.txt")
    os.makedirs(os.path.dirname(output_file_path), exist_ok=True)
    try:
        with open(output_file_path, "w") as f:
            f.write(disease_label + "\n")
        print("Output stored in:", output_file_path)
    except IOError as e:
        print(f"Unable to write to file {output_file_path}: {e}")
